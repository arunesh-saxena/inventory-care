...
export default function Settings(): JSX.Element {
    ...
    useEffect(() => {
        try {
            ...
            if (raw) {
                setValues((prev) => ({ ...prev, ...(JSON.parse(raw) as Partial<SettingsState>) }));
            }
        } catch {
            // ignore parse errors
        }
    ...
    const rowStyle: React.CSSProperties = { marginBottom: 16, display: "flex", flexDirection: "column" };
    const labelStyle: React.CSSProperties = { marginBottom: 6, fontWeight: 600, fontSize: 13 };
            ...
            <form onSubmit={handleSave} aria-label="Settings form">
                <section style={rowStyle}>
                    ...
                    <input
                        ...
                        onChange={(s) => handleChange("username", s.currentTarget.value)}
                ...
                <section style={rowStyle}>
                    ...
                    <input
                        id="email"
                        value={values.email}
                        onChange={(s) => handleChange("email", s.currentTarget.value)}
                        style={{ padding: 8, borderRadius: 4, border: "1px solid #ccc" }}
                        placeholder="you@example.com"
                        type="email"
                    />
                    {errors.email ? (
                        <div style={{ color: "crimson", marginTop: 6 }}>{errors.email}</div>
                    ) : (
                        <div style={hintStyle}>Used for notifications and account recovery</div>
                    )}
                </section>
                ...
                <section style={rowStyle}>
                    ...
                    <label style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <input
                            ...
                            onChange={(e) => handleChange("notifications", e.currentTarget.checked)}
                        />
                ...
                <section style={rowStyle}>
                    ...
                    <select
                        ...
                        onChange={(e) => handleChange("language", e.currentTarget.value as Language)}
                ...
                {status && <div style={{ marginTop: 12, color: status.includes("failed") ? "crimson" : "#007600" }}>{status}</div>}
            </form>
        </main>
    );
}